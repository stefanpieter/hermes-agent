...
class TestRuntimeBudgetConfig:
    """Local dashboard tool-output config should layer on upstream context scaling."""

    def test_context_window_scales_when_user_has_no_tool_output_overrides(self, monkeypatch):
        from tools import budget_config as mod

        monkeypatch.setattr(
            mod,
            "get_tool_output_limits",
            lambda: __import__("tools.tool_output_config", fromlist=["ToolOutputLimits"]).ToolOutputLimits(),
        )
        monkeypatch.setattr(mod, "_raw_tool_output_value", lambda key: None)
    ...
    def test_explicit_dashboard_result_threshold_overrides_context_scaled_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            mod,
            "get_tool_output_limits",
            lambda: ToolOutputLimits(result_persist_threshold_chars=12_345),
        )
        monkeypatch.setattr(
            mod,
            "_raw_tool_output_value",
            lambda key: 12_345 if key == "result_persist_threshold_chars" else None,
        )
    ...
    def test_explicit_turn_budget_overrides_only_turn_budget(self, monkeypatch):
        from tools import budget_config as mod
        from tools.tool_output_config import ToolOutputLimits
        ...
        monkeypatch.setattr(
            mod,
            "get_tool_output_limits",
            lambda: ToolOutputLimits(turn_budget_chars=55_555),
        )
        monkeypatch.setattr(
            mod,
            "_raw_tool_output_value",
            lambda key: 55_555 if key == "turn_budget_chars" else None,
        )

        cfg = mod.get_runtime_budget_config(context_length=65_536)

        assert cfg.default_result_size == int(65_536 * 4 * 0.15)
        assert cfg.turn_budget == 55_555
