...
def _collect_role_runtime_records(
    ...
    *,
    ...
    # SessionDB-backed routes can enrich sessions with role-runtime metadata.
    # Some endpoint tests intentionally swap in a tiny fake DB that only
    # implements the public listing/count API; treat role-runtime enrichment as
    # unavailable rather than making those normal list paths 500.
    if not hasattr(db, "_lock") or not hasattr(db, "_conn"):
        return []
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Kinni Dark",           "description": "Shared Kinni/Hermes dark brand palette with red and blue accents"},
