...
def list_profiles() -> List[ProfileInfo]:
    ...
    if default_home.is_dir():
        ...
        is_windows = sys.platform == "win32"
        default_alias_path = wrapper_dir / (f"{default_name}.bat" if is_windows else default_name)
        default_alias_exists = default_name != "default" and default_alias_path.exists()
        ...
        profiles.append(ProfileInfo(
            ...
            alias_path=default_alias_path if default_alias_exists else None,
            alias_name=default_name if default_alias_exists else None,
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            name = entry.name
            if _is_default_profile_name(name):
